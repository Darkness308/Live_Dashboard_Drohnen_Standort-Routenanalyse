...
class WeatherData:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "cloud_cover_percent": (
                round(self.cloud_cover_percent, 0) if self.cloud_cover_percent else None
            ),
        }
...
class GeodataService:
    ...
    def _get_nrw_loader(self) -> NRWDataLoader:
        ...
        if self._nrw_loader is None:
            self._nrw_loader = NRWDataLoader(cache_dir=self.cache_dir / "nrw")
    ...
    async def _check_dwd_status(self) -> ServiceStatus:
        ...
        try:
            ...
            request.filter_by_station_id("01078")  # Duesseldorf
    ...
    async def get_weather(
        self, lat: float, lon: float, station_id: Optional[str] = None
    ...
    async def _fetch_dwd_weather(
        self, lat: float, lon: float, station_id: Optional[str]
    ) -> Optional[WeatherData]:
        """Interne DWD-Abfrage."""
        settings = WDSettings(ts_si_units=False)
        ...
        if station_id:
            ...
            stations = request.filter_by_rank(latlon=(lat, lon), rank=1)
        ...
        return WeatherData(
            ...
            temperature_celsius=(
                float(latest.get("value", 15.0))
                if "temperature" in latest.get("parameter", "")
                else 15.0
            ),
            ...
            wind_speed_ms=(
                ...
                if "wind_speed" in latest.get("parameter", "")
                else 0
            ),
        ...
        for item in data:
            if hasattr(item, "geometry_geojson") and item.geometry_geojson:
                feature = {
                    ...
                    "properties": {
                        k: v for k, v in item.to_dict().items() if k != "geometry"
                    },
                }
...
def create_geodata_router():
    ...
    async def get_parcels(
        ...
        parcels = await service.get_parcels(
            bbox, include_geometry=include_geometry, max_features=max_features
        )
    ...
    async def get_ambient_noise(
        ...
        noise_type: str = Query(
            "strasse", description="strasse|schiene|flug|industrie"
        ),
        include_geometry: bool = Query(False),
    ):
        """Laedt Laermkartierung (Vorbelastung)."""
