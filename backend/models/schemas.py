...
class DroneModel(BaseModel):
    ...
    sound_power_level_dba: float = Field(
        ..., ge=50, le=100, description="Schallleistungspegel LwA in dB(A)"
    )
    frequency_spectrum: Optional[Dict[int, float]] = Field(
        default=None, description="Oktavband-Spektrum {Hz: dB}"
    )
...
class ImmissionsortInput(BaseModel):
    ...
    is_sensitive: bool = Field(
        default=False, description="Sensible Nutzung (Krankenhaus, Schule)"
    )
...
class NoiseCalculationRequest(BaseModel):
    ...
    # Berechnungsoptionen
    calculation_method: Literal["iso9613-2", "simplified"] = Field(default="iso9613-2")
