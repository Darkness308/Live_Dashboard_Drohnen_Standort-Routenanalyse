...
var MORPHEUS_GMAPS = (function () {
  ...
  async function init(containerId, options = {}) {
    const el = document.getElementById(containerId); if (!el) throw new Error(`Container with id "${containerId}" not found`);
    ...
    const mapOptions = {
      center: options.center || ISERLOHN_CENTER, zoom: options.zoom ?? 17, mapTypeId: google.maps.MapTypeId.SATELLITE, tilt: 45, heading: 0, mapId: config.mapId || undefined, fullscreenControl: true, zoomControl: true, rotateControl: true, streetViewControl: false,
    };
    ...
    droneMarker = new google.maps.Marker({
      ...
      title: 'Drohne (live)',
      icon: {
        path: google.maps.SymbolPath.FORWARD_CLOSED_ARROW, scale: 5, rotation: 0, fillColor: '#ff0000', fillOpacity: 0.9, strokeWeight: 1,
      },
      visible: false,
    });
    const noiseData = new google.maps.Data({ map }); noiseData.setStyle((feature) => { const level = feature.getProperty('level') || 1; const color = level >= 3 ? 'rgba(255,0,0,0.4)' : level === 2 ? 'rgba(255,165,0,0.35)' : 'rgba(0,128,255,0.25)'; return { fillColor: color, strokeColor: 'rgba(0,0,0,0.6)', strokeWeight: 1 }; }); map.__noiseDataLayer = noiseData;
  }
  function renderRoutes(routes) {
    if (!map) throw new Error('Map not initialized - call init() first'); if (!Array.isArray(routes)) return; routes.forEach((route) => {
      const id = String(route.id || route.name || Math.random()); if (routePolylines[id]) { routePolylines[id].setMap(null); delete routePolylines[id]; } const path = (route.points || []).map((p) => ({ lat: p.lat, lng: p.lng })); const polyline = new google.maps.Polyline({
        path, geodesic: true, strokeColor: route.color || '#00FF00', strokeOpacity: 0.9, strokeWeight: 3, map,
      }); polyline.__meta = { id, name: route.name || id }; routePolylines[id] = polyline; const iw = new google.maps.InfoWindow(); polyline.addListener('click', (ev) => { iw.setContent(`<div><strong>${polyline.__meta.name}</strong><br/>Points: ${path.length}</div>`); iw.setPosition(ev.latLng); iw.open(map); });
    });
  }
  ...
  return {
    init, renderRoutes, renderNoiseZones, updateDronePosition, handleLiveUpdate,
  };
}());
